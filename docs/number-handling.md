...
| 手里是 `bson.Long` / `Int32` 等 | Handling **不会**把它们拆成字面量；`none` 会抛。要裸数字请让驱动 `promoteLongs` / `promoteValues` / `useBigInt64`。见 [behavior-matrix.md](../packages/tason-mongodb/behavior-matrix.md) |
