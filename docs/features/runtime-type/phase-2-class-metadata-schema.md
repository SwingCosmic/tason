...
| **2.2** | 递归遍历（array / 嵌套 object）+ 全数值边界 + `object-type-property` 上下文 | 多实现解析、无 schema 时记住 TypeName、override / brand（见阶段 3 / 2.2b） | **done** |
...
| `object-fallback-all` | 降级 | 仍降级 | **按设计语义实现**（有契约时 schema 优先） |
...
> **已完成。** 目录重构、Valibot adapter、ClassMetadata、叶子映射与 S0–S9 测试已落地。
...
| tree-shaking / 体积 | 优 |
| 结构解析 | 标准 schema 树可遍历 |
| `bigint` | `v.bigint()` 一等支持 |
| 绑定 | optional peer；core 不硬依赖 |

```jsonc
// package.json（示意；实施时锁定主版本）
{
...
- 启用：`registry.setSchemaAdapter(createValibotAdapter())`。
...
| 嵌套 object / 多维 array 映射 | adapter 可遍历 | Visitor / Generator 递归 |
| optional / nullable | 尽量 | 完善 |
| decimal kind | 可识别 | 映射完备 |

**Valibot（`createValibotAdapter()`）：** 识别 schema；遍历 object / array；叶子 kind 映射；optional 尽量解包。

---

## 2.1.4 元数据与 adapter 入口

公开 **TASON 自身 API**，不把外部库细节做成公开接口：
...
| 项 | 决策 |
| --- | --- |
| 存储 | **R1**：挂在 Registry 条目上（与 register 同生命周期）。不用 R2 全局 WeakMap（JS 没有 .NET Type 表） |
| 默认 adapter | **不注册** |
...
**反序列化：** 属性包 → `getClassMetadata` → adapter 遍历 → 按 kind `mapTypeInstanceToRuntime` → `createInstance`。  
**序列化：** 识别类型 → metadata + adapter 遍历 → 按 kind + serialize Handling 写出类型名或字面量。
**回退：** 无 metadata / 无 adapter / `unknown` → 阶段 1 值级。

**2.1 不做：** 多维数组元素按契约转换、object-type-property 取消降级、字段类型名 override、强制 `v.parse`。

### 叶子映射子集

| RuntimeType | TypeName / 字面量（例） | object-fallback-native 转换结果 | serialize（unsafe-only） |
| --- | --- | --- | --- |
| bigint | `Int64("1")` | `1n` | 安全范围内写裸字面量，否则类型名装箱 |
...
| F1–F2 | 新开阶段 2 测试；写清 `setSchemaAdapter` 用法 | `test/runtime-schema*.test.ts` |
...
| 未实现默认通过 | 2.2 用例 `it.skip` / `it.todo`，2.2 再启用 |
...
- [x] adapter + metadata 时叶子按契约转换 / 写出
- [x] 无 adapter / 无 metadata / `native` 不引入回归  
- [x] 阶段 2 测试场景已写入；2.2 用例当时以 skip 默认通过  
- [x] 文档：`setSchemaAdapter(createValibotAdapter())` 启用方式
...
> **已完成。** 对照 C#：`FillObjectMembers` + `TypedValueContext` + `ObjectTypeProperty` / `ValueScope`；JS 用 **遍历 schema + map\*** + `inObjectType` 上下文。
...
**2.2 不做（2.2b / P2 / 阶段 3）：** 字段级 TypeName override、Valibot brand 路径、反序列化后强制 `v.parse`、无 schema 时“记住数组元素 TypeName”的额外状态、多实现选型、多维数组压力测试。
...
| 能力 | 位置 | 2.1 状态 |
| --- | --- | --- |
| Adapter 接口 `arrayElement` / `objectEntries` / unwrap optional | `RuntimeSchemaAdapter` + `adapters/valibot.ts` | **已有**（Valibot 可遍历数组与包装层） |
...
| G7 | S10–S17 无断言 | `expect(true).toBe(false)` 占位 | 真实场景 + `PHASE_2_2 = true` |
...
—— 第二参由 Visitor / Generator 在 ObjectType 路径传入；**缺省**保持阶段 1 降级，保证旧测试与无上下文路径不回归。
...
\* **S16 默认策略（2.2 确定）：** 契约 `number` + `bigint` / `Int64` 仅当 `Number.isSafeInteger(Number(bi))` 且往返不丢精度时转为 number；否则抛错。不静默截断。
...
3. `test: enable phase 2.2 runtime-schema S10–S17`
...
| **C4** | 元素 / 字段 `unknown` 或 adapter 失败 → 透传，不抛 | 同上 | 与 S5 同样处理 |
...
| 项 | 去向 |
| --- | --- |
| adapter 重写 / 第二 schema 库 | 不做；Valibot 已能遍历 |
...
| **S13** | 单 ObjectType 多字段：UInt8…Decimal128 / BigInt 与契约 bigint\|number\|decimal 交叉抽样 | object-fallback-native 下按覆盖矩阵转换；stringify 抽样符合 serialize Handling |
...
| **S16** | 契约 `number` + `Int64` 安全整数 → number；`Int64` 超 `MAX_SAFE_INTEGER` → **抛错** | 锁定 B1 |
