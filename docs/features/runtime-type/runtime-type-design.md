...
设计范围：数值处理、实体元数据、鸭子类型 / 多态。  
进度见 [implementation-plan.md](./implementation-plan.md)；分册：[阶段 1](./phase-1-number-handling.md) · [阶段 2](./phase-2-class-metadata-schema.md) · [阶段 3](./phase-3-duck-types.md)。规范见 [type-system.md](../../type-system.md)。
...
| 动态结构 | 无 schema 时，类型只能从值本身或 Registry 推断 |
| 实体 / 接口约定 | 需要 Schema 表达 RuntimeType 形状（嵌套、数组元素是 bigint 还是 number） |
| 两套类型交叉 | TypeName ↔ RuntimeType 多对多（见 [术语与用语](../glossary.md)），需要默认映射 + Handling / 多实现选型 |
...
```
  应用接口约定（RuntimeType）     TASON 文本（TypeInstance / 字面量）
  ┌─────────────────────┐       ┌──────────────────────┐
...
ClassMetadata 用**现成轻量库的 Schema**表达 **RuntimeType**，作为**接口约定**；  
不再另写一套 TASON 声明类型（那是 Registry / TypeName 的职责）。
...
2. 与 **TypeName / TypeInstance** 的**交叉映射**（本库独有，映射逻辑应尽量少）
...
- 结构遍历 / 校验：库自带；TASON 只在叶子做数值映射

### 4.3 RuntimeType ↔ TypeInstance / 字面量：交叉映射与默认路径

两套体系**交叉**，不是粗细包含（详见 [术语与用语](../glossary.md)）：

- **TypeName → 多 RuntimeType**：鸭子类型（同一 TypeName 多种 JS 实现）
- **RuntimeType → 多 TypeInstance / 字面量**：如 `bigint` 可对应 `Int64("…")`、`BigInt("…")` 或裸字面量

序列化 / 反序列化需要**默认路径**（下表）+ Number Handling；换路径用 override / brand / 追加类型实现 / `all` 等——**不要**为此重做整棵 TypeName 树。
...
**字面量的含义：** JSON 风格字面量本身已是准确类型——字符串**只**是字符串，**不得**把 `"1"`、RFC3339、`"/a/"` 等字符串字面量解释成 number / Date / RegExp（那是 JSON 无法表达类型时的折中，TASON 不用）。
`Date` / `RegExp` 等只能来自 **TypeInstance**（或已是该 ctor 的实例）。

### 4.4 `object-fallback-native` 在本模型下

- **命名含义**：优先用 schema 契约解释字段（有则按 RuntimeType 转换），无契约则退回 native 拆箱——**不是**把 TypeName 记到别处再读回来
- 应用层拿到的是：拆箱后的 number / bigint / Decimal（或契约指定的 RuntimeType）
- 有 Schema 时：按 **期望 RuntimeType** 转换（`Int64("1")` + bigint → `1n`；其它 TypeName 路径也可以收敛到同一 RuntimeType）
- 数组 / 嵌套按 Schema 遍历，保证 `bigint[]` 的元素真是 bigint
- 再次序列化靠序列化侧 Handling + schema 期望的 kind，而不是 parse 时记住的 TypeName
...
| **已决议** | 导出 Valibot 实现，可直接 `setSchemaAdapter` 启用；**默认不注册**任何 adapter |
| 公开元数据 API | **`getClassMetadata`**（整包）；**不**提供 `getSchema`（避免把外部库细节做成公开 API） |
| 存储 | 挂在 Registry 条目上（R1）；JS 没有 .NET 那种全局 Type 表，不模仿 Provider |
| 无 adapter / 无 metadata | 行为与现在相同，只走 Number Handling / 多实现 |
...
| Schema 结构解析 | （反射 PropertyType） | `src/schema/` + Valibot adapter |
...
| 默认交叉映射路径 | 完整 brand / Int32 枚举体系（需要时再加 override 即可） |
...
parse / parseAs(User)
  ...
  → 有 Schema：按期望 RuntimeType 转换（object-fallback-native）
...
| **3** | `asDefault`/`setDefaultType`、`parseAs`、`getTypeInfoByCtor` | [phase-3](./phase-3-duck-types.md) **done** |
...
2. **ClassMetadata = Schema（现成库）**，作为接口约定（对照 C# 反射元数据）。
3. **Registry + Number Handling** 在交叉关系上选定 **TypeName / TypeInstance 路径**；中间只做少量映射。
4. 嵌套 / 多维数组交给 schema 库，不自研类型 DSL。
5. `number` 默认不写成 Int32；其它 TypeName 路径用 override / brand / `all` / 追加类型实现。
6. 核心 adapter 接口 + 官方 **Valibot 实现导出**；默认不注册，调用 `setSchemaAdapter` 即可启用。
7. 公开 `getClassMetadata`，不公开 `getSchema`。
8. 鸭子类型仍在 Registry 层。
9. 目录：`TASONTypeInfo` 放到 `src` 根下，`metadata/` 与 `schema/` 分开（阶段 2.1）。
10. 功能自 **tason-net** 迁移；ExtraMember / naming / Enum 元数据本轮不迁。
11. 阶段 2 测试新开文件覆盖全场景；未实现部分用 skip / todo，默认通过。  
